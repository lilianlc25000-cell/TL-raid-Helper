...
import RollManagerClient from "../../[id]/RollManagerClient";

type PageProps = {
  params: { id: string };
};

export default async function ActiveLootRollManagerPage({ params }: PageProps) {
      ...
      <div className="min-h-screen bg-zinc-950 px-6 py-10 text-zinc-100">
        <div className="mx-auto max-w-3xl rounded-lg border border-red-500/40 bg-red-950/30 px-6 py-6 text-sm text-red-200">
          {sessionError?.message
            ? `Impossible de charger cette session: ${sessionError.message}`
            : "Session de loot introuvable ou accès refusé."}
        </div>
      </div>
    );
  }
