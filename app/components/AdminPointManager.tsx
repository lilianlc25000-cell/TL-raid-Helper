...
export default function AdminPointManager({ user }: AdminPointManagerProps) {
  ...
  return (
    <section className="relative w-full max-w-3xl rounded-lg border border-zinc-800 bg-zinc-950 text-zinc-100 shadow-[0_0_25px_rgba(0,0,0,0.4)]">
      <header className="flex flex-col gap-3 border-b border-zinc-800 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div>
      ...
      {toast ? (
        <div className="absolute right-4 top-4 max-w-[calc(100%-2rem)] rounded-md border border-emerald-700 bg-emerald-950 px-4 py-2 text-sm text-emerald-200 shadow-lg sm:right-6 sm:top-6 sm:max-w-none">
          {toast}
        </div>
      ) : null}
    </section>
